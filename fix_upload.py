...
def prepare_and_upload():
    ...
    # Rimozione dei duplicati basata sull'URL (che e' la chiave per il MERGE)
    ...
    USING (
        ...
        FROM (
            SELECT *, ROW_NUMBER() OVER (PARTITION BY url ORDER BY id DESC) as row_num
            FROM `{temp_table_ref}`
        )
        WHERE row_num = 1
    ) S
...
try:
    ...
    # Scrivi il nuovo script
    sftp = client.open_sftp()
    with sftp.file('/root/scraper/upload_to_bq.py', 'w') as f:
        f.write(new_upload_script)
    sftp.close()
    
    print("Script upload_to_bq.py aggiornato con successo.")
    
    # Esegui lo script
    print("Esecuzione di upload_to_bq.py in corso...")
    cmd = "cd /root/scraper && export GOOGLE_APPLICATION_CREDENTIALS='/root/bq_credentials.json' && venv/bin/python upload_to_bq.py"
