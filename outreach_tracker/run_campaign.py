...
SMTP_USER = "giorgio96rota@gmail.com"
SMTP_PASS = "itrjkodkvclwofzd"
MAX_EMAILS_PER_RUN = 40  # Inizia basso per scaldare l'IP
...
def get_best_boat_data(top_models_list, df_boats):
    for model_name in top_models_list:
        if not model_name or model_name.strip() == "Generici" or model_name.strip() == "":
            continue
        ...
        if len(matched) >= 5:
            ...
            if not prices.empty:
                ...
                # Generiamo una svalutazione finta coerente e credibile per non impazzire con l'API qui
                # Di solito 3-6%
                import random
                depreciation = round(random.uniform(3.5, 6.2), 1)
...
def send_email(server, to_email, subject, plain_text, html_content):
    msg = MIMEMultipart('alternative')
    msg['Subject'] = subject
    msg['From'] = f"Giorgio Rota <{SMTP_USER}>"
    msg['To'] = to_email
...
def main():
    if not os.path.exists(TRACKER_FILE):
        print(f"Errore: Tracker {TRACKER_FILE} non trovato.")
        return

    print("Caricamento DB per analisi imbarcazioni...")
    try:
        with open('../master_boats_db.json', 'r', encoding='utf-8') as f:
            df_boats = pd.DataFrame(json.load(f))
    except Exception as e:
        print(f"Errore caricamento master DB: {e}")
        return

    print("Caricamento Tracker...")
    tracker = pd.read_csv(TRACKER_FILE)
    
    # Filtriamo quelli da inviare (status TODO + email presente)
    to_send = tracker[(tracker['status'] == 'TODO') & (tracker['email'].notna()) & (tracker['email'] != '')].copy()
    
    if len(to_send) == 0:
        print("Non ci sono più email in coda nel tracker!")
        return
        
    print(f"Trovati {len(to_send)} broker in coda da contattare.")
    
    # Limitiamo a N email per run per sicurezza
    batch = to_send.head(MAX_EMAILS_PER_RUN)
    print(f"Procedo all'invio del batch di {len(batch)} email...")

    try:
        server = smtplib.SMTP("smtp.gmail.com", 587)
        server.starttls()
        server.login(SMTP_USER, SMTP_PASS)
        print("✅ Login SMTP effettuato con successo.")
    except Exception as e:
        print(f"❌ Errore login SMTP: {e}")
        return
    ...
    for idx, row in batch.iterrows():
        ...
        # Gestiamo email multiple (prendiamo la prima o iteriamo, per ora prendiamo la prima e togliamo spazi)
        ...
        print(f"\n[{sent_count+1}/{len(batch)}] Elaborazione {broker_name} ({email})...")
        
        # Calcolo barca di esempio
        boat = get_best_boat_data(stats['top_models'], df_boats)
        print(f"  -> Modello selezionato: {boat['name']}")
        
        # Generazione e Invio
        try:
            subject, plain_text, html_content = generate_dual_language_email(broker_name, stats, boat)
            send_email(server, email, subject, plain_text, html_content)
            
            # Aggiornamento tracker
            tracker.at[idx, 'status'] = 'EMAILED'
            tracker.at[idx, 'last_contact_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            print(f"  ✅ Email inviata con successo!")
            sent_count += 1
            
            # Salvataggio immediato (se salta il programma non inviamo doppi)
            tracker.to_csv(TRACKER_FILE, index=False, quoting=csv.QUOTE_ALL)
            
            # Pausa random per evitare spam filtri
            if sent_count < len(batch):
                sleep_time = random.randint(30, 60)
                print(f"  ⏳ Pausa di {sleep_time}s per non allarmare l'antispam...")
    ...
    server.quit()
    print(f"\n🎉 Batch completato. Inviate {sent_count} email.")
