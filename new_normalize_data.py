...
def load_data():
    files = [
        ...
        "yachtall_boats.json",
        "topboats_data.json",
        "navisnet_mapped_preview.json" # Aggiunto Navisnet
    ]
    all_data = []
    for f in files:
        try:
...
def normalize_countries(df):
    logging.info("Starting country normalization...")
    
    country_map = {
        'albania': 'AL', 'andorra': 'AD', 'antigua e barbuda': 'AG', 'antigua-e-barbuda': 'AG',
        'antille olandesi': 'AN', 'antille-olandesi': 'AN', 'arabia-saudita': 'SA', 'argentina': 'AR',
        'aruba': 'AW', 'australia': 'AU', 'austria': 'AT', 'bahamas': 'BS', 'bahrain': 'BH',
        'bangladesh': 'BD', 'belgio': 'BE', 'belize': 'BZ', 'bosnia-erzegovina': 'BA', 'brasile': 'BR',
        'bulgaria': 'BG', 'canada': 'CA', 'cile': 'CL', 'cina': 'CN', 'cipro': 'CY', 'cocos-islands': 'CC',
        'colombia': 'CO', 'corea del sud': 'KR', 'corea-del-sud': 'KR', 'costa rica': 'CR', 'costa-rica': 'CR',
        'croazia': 'HR', 'cuba': 'CU', 'curacao': 'CW', 'curaçao': 'CW', 'danimarca': 'DK', 'dominica': 'DM',
        'ecuador': 'EC', 'egitto': 'EG', 'emirati arabi uniti': 'AE', 'emirati-arabi-uniti': 'AE',
        'estonia': 'EE', 'figi': 'FJ', 'fiji': 'FJ', 'filippine': 'PH', 'finlandia': 'FI', 'francia': 'FR',
        'french-polynesia': 'PF', 'georgia': 'GE', 'germania': 'DE', 'giappone': 'JP', 'gibraltar': 'GI',
        'giordania': 'JO', 'gran bretagna': 'GB', 'grecia': 'GR', 'grenada': 'GD', 'guadalupa': 'GP',
        'guatemala': 'GT', 'guernsey': 'GG', 'hong-kong': 'HK', 'indonesia': 'ID', 'irlanda': 'IE',
        'isole-cayman': 'KY', 'isole cook': 'CK', 'isole-turks-e-caicos': 'TC', 'isole vergini americane': 'VI',
        'isole vergini britanniche': 'VG', 'isole-vergini-regno-unito': 'VG', 'isole-vergini-usa': 'VI',
        'israele': 'IL', 'italia': 'IT', 'jamaica': 'JM', 'jersey': 'JE', 'kenya': 'KE', 'kuwait': 'KW',
        'lettonia': 'LV', 'libano': 'LB', 'lituania': 'LT', 'lussemburgo': 'LU', 'macao': 'MO', 'macau': 'MO',
        'madagascar': 'MG', 'malesia': 'MY', 'malta': 'MT', 'marocco': 'MA', 'martinica': 'MQ', 'mauritania': 'MR',
        'mauritius': 'MU', 'messico': 'MX', 'monaco': 'MC', 'montenegro': 'ME', 'myanmar': 'MM',
        'new-caledonia': 'NC', 'norvegia': 'NO', 'nuova caledonia': 'NC', 'nuova zelanda': 'NZ',
        'nuova-zelanda': 'NZ', 'olanda': 'NL', 'paesi bassi/holanda': 'NL', 'palau': 'PW', 'panama': 'PA',
        'panamá': 'PA', 'paraguay': 'PY', 'peru': 'PE', 'polinesia francese': 'PF', 'polonia': 'PL',
        'portogallo': 'PT', 'porto-rico': 'PR', 'qatar': 'QA', 'regno unito': 'GB', 'regno-unito': 'GB',
        'repubblica ceca': 'CZ', 'repubblica-ceca': 'CZ', 'repubblica-dominicana': 'DO', 'romania': 'RO',
        'russia': 'RU', 'saint-barthelemy': 'BL', 'saint-barthélemy': 'BL', 'saint-lucia': 'LC',
        'saint-martin': 'MF', 'saint vincent e grenadine': 'VC', 'saint-vincent-e-le-grenadines': 'VC',
        'santa lucia': 'LC', 'serbia': 'RS', 'serbia-e-montenegro': 'CS', 'seychelles': 'SC',
        'singapore': 'SG', 'sint maarten': 'SX', 'slovacchia': 'SK', 'slovenia': 'SI', 'spagna': 'ES',
        "stati uniti d'america": 'US', 'stati uniti': 'US', 'sud-africa': 'ZA', 'sudafrica': 'ZA',
        'svezia': 'SE', 'svizzera': 'CH', 'tailandia': 'TH', 'taiwan': 'TW', 'tanzania': 'TZ',
        'thailandia': 'TH', 'trinidad e tobago': 'TT', 'trinidad-e-tobago': 'TT', 'tunisia': 'TN',
        'turchia': 'TR', 'turks e caicos': 'TC', 'ucraina': 'UA', 'ungheria': 'HU', 'uruguay': 'UY',
        'usa': 'US', 'uk': 'GB'
    }
    ...
    def map_country(c):
        ...
        # Se è già un codice ISO a due lettere, lascialo così (uppercase)
        if len(c_lower) == 2:
            return c_lower.upper()
        return country_map.get(c_lower, c.title())
...
def clean_and_unify_data():
    ...
    # Aggiungi category se mancante dal json originale
    ...
    columns_to_keep = [
        ...
        'price_eur', 'length', 'image_url', 'source', 'broker', 'url', 'category'
    ]
    
    # Tieni solo le colonne che esistono effettivamente per non far crashare pandas
    actual_columns = [col for col in columns_to_keep if col in df.columns]
    
    df_final = df[actual_columns]
    if 'builder_canonical' in df_final.columns:
        df_final = df_final.rename(columns={'builder_canonical': 'builder'})
